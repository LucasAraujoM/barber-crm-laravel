...
<div
    x-data="toastManager()"
    x-on:toast.window="show($event.detail.message, $event.detail.type ?? 'warning')"
    ...
    <template x-for="(t, i) in toasts" :key="t.id">
        <div
            ...
            {{-- mensaje --}}
            <p class="flex-1 text-sm font-semibold leading-snug" x-text="t.message"></p>
            ...
            {{-- barra de progreso --}}
            <div class="absolute bottom-0 left-0 h-[3px] rounded-full"
                 :class="{
                     'bg-amber-400': t.type === 'warning',
                     'bg-[#F5F7FA]':  t.type !== 'warning',
                 }"
