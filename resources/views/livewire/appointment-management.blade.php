    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
            <div class="bg-[#3B82F6] p-3 rounded-2xl shadow-md border border-[#2563EB] flex items-center gap-3 text-white">
                ...
                <div>
                    <p class="text-[10px] font-black uppercase tracking-widest opacity-80 mb-0.5">Total Histórico</p>
            ...
            <div class="bg-white p-3 rounded-2xl shadow-sm border border-[#E5E7EB] flex items-center gap-3">
                ...
                <div>
                    <p class="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-0.5">Hoy</p>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-4">
            {{-- Calendario (Col 2/3) --}}
            <div class="xl:col-span-2 space-y-4" x-data="{
                events: @js($events),
                calendar: null,
                initCalendar() {
               ...
               @calendar-updated.window="refreshCalendar()">
                <div class="bg-white rounded-[1.25rem] shadow-sm border border-[#E5E7EB] p-4 shrink-0">
                    ...
                    <div class="w-full min-h-[680px]" x-ref="calendarEl" wire:ignore></div>
                </div>
            </div>
            ...
            <div class="space-y-4">
                <div class="bg-white rounded-[1.25rem] shadow-sm border border-[#E5E7EB] overflow-hidden shrink-0">
                    ...
                    <div class="p-4 space-y-3">
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5 block">Desde</label>
                                <input type="date" wire:model.live="date_from" class="w-full bg-white border border-[#E5E7EB] rounded-xl py-2 px-3 text-xs font-bold text-gray-500 focus:ring-2 focus:ring-indigo-100 outline-none">
                            </div>
                            <div>
                                <label class="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5 block">Hasta</label>
                                <input type="date" wire:model.live="date_to" class="w-full bg-white border border-[#E5E7EB] rounded-xl py-2 px-3 text-xs font-bold text-gray-500 focus:ring-2 focus:ring-indigo-100 outline-none">
                            </div>
                        </div>
                        <div>
                            <label class="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5 block">Barbero</label>
                            <select wire:model.live="staff_id" class="w-full bg-white border border-[#E5E7EB] rounded-xl py-2 px-3 text-xs font-bold text-gray-500 focus:ring-2 focus:ring-indigo-100 outline-none">
                ...
                <div class="bg-white rounded-[1.25rem] shadow-sm border border-[#E5E7EB] overflow-hidden shrink-0 flex flex-col h-[400px]">
                    ...
                    <div class="divide-y divide-[#E5E7EB] flex-1 overflow-y-auto custom-scrollbar">
                        @forelse($appointments as $appt)
                            <div class="p-4 hover:bg-slate-50/50 transition-all group">
                    ...
                    @if($appointments->hasPages())
                        <div class="p-4 bg-white border-t border-[#E5E7EB]">
                            {{ $appointments->links() }}
                        </div>
                    @endif
                </div>
        ...
        <div x-data="{ show: $wire.entangle('showModal') }" x-show="show" style="display:none" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div x-show="show" x-transition.scale class="relative w-full max-w-lg bg-white rounded-[2.5rem] shadow-2xl z-10 overflow-hidden">
                <div class="p-8">
